...
<tr class="nav border-underline transition-all duration-300">
    <td class="py-4 px-2 sm:px-6 font-medium textp text-center sm:text-left">
        <div class="flex items-center gap-1 ml-4 sm:ml-6 justify-start">
            #NAME#
            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="16px" fill="" class="filter-color cursor-pointer info-icon" onclick="showCardPaid()">
...
const cardPaid = `
<div id="cardPaid" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden">
    <div class="nav rounded-lg p-8 max-w-md w-full mx-4 transform transition-all">
        <div class="text-center">
            ...
            <button onclick="hideCardPaid()" class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-md transition-colors">
                Close
            </button>
        </div>
    </div>
...
document.body.insertAdjacentHTML('beforeend', cardPaid);

window.showCardPaid = function() {
    document.getElementById('cardPaid').classList.remove('hidden');
};

window.hideCardPaid = function() {
    document.getElementById('cardPaid').classList.add('hidden');
};